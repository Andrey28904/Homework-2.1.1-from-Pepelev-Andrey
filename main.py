...
class Report:
    def __init__(self, data: DataSet):
        side = Side("thin")
        self.bold_border = Border(top=side, bottom=side, left=side, right=side)
        ...
        self.sheet_1_values = [list(data.year_to_salary.keys()), list(data.year_to_salary.values()),
        ...
        self.sheet_2_values = [list(data.area_to_salary.keys()), list(data.area_to_salary.values()), [],
                               list(data.area_to_piece.keys()), list(data.area_to_piece.values())]

    def get_max_col_width(self, values: list):
        to_str = map(str, values)
        return max(map(len, to_str)) + 2

    def handle_sheet(self, wb: Workbook, ws: Worksheet, headers: list, values: list):
        for i in range(len(headers)):
            cell = ws.cell(row=1, column=i+1, value=headers[i])
            if cell.value != "":
                cell.border = self.bold_border
            cell.font = Font(bold=True)
        for i in range(len(values)):
            for t in range(len(values[i])):
                cell = ws.cell(row=t+2, column=i+1, value=values[i][t])
                cell.border = self.bold_border
                if headers[i] == "Доля вакансий":
                    cell.number_format = BUILTIN_FORMATS[10]
            ws.column_dimensions[get_column_letter(i+1)].width = self.get_max_col_width(values[i] + [headers[i]])

    def generate_excel(self, file_name: str):
        wb = Workbook()
        ws = wb.worksheets[0]
        ws.title = "Статистика по годам"
        self.handle_sheet(wb, ws, self.sheet_1_headers, self.sheet_1_values)
        ws = wb.create_sheet("Статистика по городам", 1)
        self.handle_sheet(wb, ws, self.sheet_2_headers, self.sheet_2_values)
        wb.save(file_name)


report_data = Report(DataSet())
report_data.generate_excel("report.xlsx")